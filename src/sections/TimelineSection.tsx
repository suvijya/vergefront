...
export default function TimelineSection() {
    ...
    useEffect(() => {
        const ctx = gsap.context(() => {
            ...
            if (!section || !content || !activeLine) return;

            // Calculate how far to translate the content
            const getScrollAmount = () => {
                const wrapper = wrapperRef.current;
                const contentHeight = content.scrollHeight;
                const wrapperHeight = wrapper ? wrapper.clientHeight : window.innerHeight;
                return Math.max(0, contentHeight - wrapperHeight + 80);
            };

            // Main scrub animation: pin the section and scroll the content upward
            ...
            // Spine glow fill tied to the same scroll range
    ...
    return (
        <section id="schedule" ref={sectionRef} className="relative w-full bg-black pt-0 pb-16 overflow-hidden h-screen flex flex-col items-center">
            ...
            {/* Header - Fixed inside the section during pinning */}
            <AnimatedSection direction="up" duration={0.75} delay={0} className="relative z-10 text-center mb-4 px-4 flex-shrink-0">
            ...
            {/* Timeline Wrapper (masks the moving content) */}
            <div ref={wrapperRef} className="relative w-full max-w-5xl flex-1 overflow-hidden px-4 md:px-0 z-0">

                {/* Scrollable Content */}
                <div ref={contentRef} className="relative pb-16 pt-8 w-full flex flex-col">
                    ...
                    <div className="absolute left-6 md:left-1/2 top-0 bottom-0 w-[2px] bg-white/5 md:-translate-x-1/2">
                        <div ref={activeLineRef} className="w-full h-0 relative bg-gradient-to-b from-cyan-400 via-emerald-400 to-violet-500 shadow-[0_0_20px_rgba(56,189,248,0.6)]">
                            <div className="absolute top-full left-1/2 -translate-x-1/2 w-2 h-2 bg-white rounded-full shadow-[0_0_10px_rgba(255,255,255,0.8),0_0_20px_rgba(56,189,248,0.8)] animate-pulse -mt-1"></div>
                        </div>
                    </div>

                    {scheduleData.map((day, dayIndex) => {
                        return (
                            <div key={dayIndex} className="w-full flex flex-col mb-8">
                                {/* Day Header/Divider */}
                                <div className="relative z-20 flex justify-center w-full mb-6 mt-4">
                                    <div className="px-6 py-2 rounded-full border border-emerald-500/30 bg-black/80 backdrop-blur-md text-emerald-400 font-mono text-sm tracking-widest uppercase shadow-[0_0_15px_rgba(16,185,129,0.2)]">
                                        {day.date}
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-col gap-4 md:gap-8">
                                    {day.events.map((event, index) => {
                                        // Reset alternating per day: index 0 is always Right Side (true).
                                        const isRightSide = index % 2 === 0;

                                        return (
                                            <div key={index} className={`relative flex flex-col md:flex-row items-center w-full ${isRightSide ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
                                                ...
                                                {/* Connector */}
                                                <div className={`hidden md:block absolute top-1/2 h-[1px] z-10 timeline-connector ${isRightSide ? 'right-1/2 mr-[6px]' : 'left-1/2 ml-[6px]'}`}
                                                    style={{
                                                        width: '40px',
                                                        background: `linear-gradient(${isRightSide ? 'to left' : 'to right'}, rgba(16,185,129,0.3), transparent)`,
                                                        transformOrigin: isRightSide ? 'right' : 'left',
                                                    }}
                                                />
                                                ...
                                                {/* Event Card */}
                                                <div className={`w-full md:w-1/2 pl-14 md:pl-0 ${isRightSide ? 'md:pr-16 md:text-right' : 'md:pl-16 md:text-left'}`}>
                                                    <div
                                                        data-side={isRightSide ? 'right' : 'left'}
                                                    ...
                                                    >
                                                        ...
                                                        <div className={`flex items-start gap-4 ${isRightSide ? 'md:flex-row-reverse' : 'md:flex-row'}`}>
                                                            ...
                                                            <div className={`flex flex-col flex-1 ${isRightSide ? 'md:items-end' : 'md:items-start'}`}>
            ...
            <div className="absolute bottom-0 w-full h-[1px] bg-gradient-to-r from-transparent via-emerald-500/20 to-transparent z-10"></div>
